...
\caption{Torre de giro central: placa base + rodamiento de giro (pistas y bolas) + carro
giratorio con brazo radial y poste; el pedestal central fijo sube por el eje del rodamiento y
sostiene la plataforma (cuenco) arriba.}
\label{fig:bearing}
\end{figure}

\section{Torre de giro: rodamiento central y caminos de carga}
La observaci\'on inicial era que, en el Omni One, el anillo giratorio del brazo y la plataforma
parec\'ian compartir el \emph{mismo rodamiento central} sobre una base estructural. El dise\~no
de OpenOmni lo resuelve con un \textbf{rodamiento de giro} (\emph{slewing ring}: pista externa
fija, pista interna giratoria y bolas intermedias) y, sobre todo, \textbf{separando dos caminos
de carga} para que ninguna funci\'on comprometa a la otra:
\begin{itemize}\itemsep2pt
\item \textbf{Peso del usuario} ($W$): cuenco $\rightarrow$ \emph{pedestal central fijo} (que sube
por el \emph{bore} del rodamiento) $\rightarrow$ placa base $\rightarrow$ piso. El rodamiento
\emph{no} soporta el peso axial del usuario.
\item \textbf{Giro del brazo}: carro giratorio $\rightarrow$ pista interna $\rightarrow$ bolas
$\rightarrow$ pista externa fija $\rightarrow$ placa base $\rightarrow$ piso. El rodamiento
toma \emph{s\'olo} el giro y el momento de vuelco del brazo,
$M \approx P_h\,h \lesssim \SI{600}{\newton}\cdot\SI{1.0}{\meter}$ (Secci\'on~6).
\end{itemize}
El apilado, de abajo hacia arriba, es: \textbf{placa base con patas} (anti-vuelco) $\cdot$
\textbf{rodamiento} (pistas + bolas) $\cdot$ \textbf{brazo/carro} $\cdot$ \textbf{pedestal} $\cdot$
\textbf{plataforma (cuenco) arriba}. Un \'unico \emph{slewing ring} tambi\'en podr\'ia cargar
ambas funciones ---est\'an dise\~nados para carga combinada axial+radial+momento---, pero
separar los caminos abarata la pieza (basta un rodamiento de menor capacidad axial) y desacopla
el desgaste del giro de la seguridad estructural bajo el usuario.
\vspace{2pt}

\noindent\fbox{\begin{minipage}{0.95\linewidth}\small
\textbf{Pendiente de validar en prototipo:} rigidez del voladizo del cuenco sobre el pedestal
(el borde a $r=R$ genera momento sobre el eje central), holgura/pre\-carga del rodamiento
para un giro suave sin juego, y verificaci\'on experimental del reparto real del rodamiento
del Omni One frente a esta arquitectura de caminos separados.
\end{minipage}}
\vspace{4pt}
